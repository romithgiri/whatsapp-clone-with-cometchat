...
import com.cometchat.ui.activities.CreateConversationGroup
import com.cometchat.ui.activities.CreateConversationGroup.Companion.selectionTrackerTempList
import com.squareup.picasso.Picasso
import java.io.InputStream
import java.net.URL


class SelectedUsersForGroupConversationAdapter(
    private val context: Context,
    private val modelList: MutableList<SelectedUsersForGroupConversationModel>
) : RecyclerView.Adapter<SelectedUsersForGroupConversationAdapter.ViewHolder>() {
    private val TAG = "SelectedUsersForGroupConversationAdapter"
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        holder.view.setOnClickListener {
            ...
            holder.view.setBackgroundColor(if (model.isSelected) {
                var tempIndex = -1
                if (!selectionTrackerTempList.isNullOrEmpty()) {
                    for (i in 0 until selectionTrackerTempList.size){
                        if(model.uid == selectionTrackerTempList[i].uid){
                            tempIndex = i
                            break
                        }
                    }
                    if (tempIndex == -1){
                        selectionTrackerTempList.add(model)
                    }
                } else {
                    selectionTrackerTempList.add(model)
                }
                ...
                if (!selectionTrackerTempList.isNullOrEmpty()) {
                    ...
                    if (tempIndex != -1){
                        selectionTrackerTempList.removeAt(tempIndex)
                    }
                }
                Color.TRANSPARENT
            })
